...
策略 III 是實盤採用的第二軌（`index.html` 雙軌顯示之一），策略 II/IV/V 只出現在 `comparison.html` 的五策略回測對照。這四個都是「單日跌幅」觸發，差別只在跌幅門檻與出場門檻：

| 策略 | 進場（第一批） | 加碼（第二批） | 出場 |
|---|---|---|---|
| II | 單日跌幅 > 1% → 投資本池 × 0.5 | 單日跌幅 > 3% → 投剩餘資本池 | 損益 ≥ +2% 全數出場 |
| III | 單日跌幅 > 1% → 投資本池 × 0.5 | 單日跌幅 > 3% → 投剩餘資本池 | 損益 ≥ +3% 全數出場 |
| IV | 單日跌幅 > 1.5% → 投資本池 × 0.5 | 單日跌幅 > 3.5% → 投剩餘資本池 | 損益 ≥ +2% 全數出場 |
| V | 單日跌幅 > 1.5% → 投資本池 × 0.5 | 單日跌幅 > 3.5% → 投剩餘資本池 | 損益 ≥ +3% 全數出場 |

沒有 MA60 警戒訊號（僅策略 I 有）。原始邏輯見 `src/strategy_iii.py`（策略 III）與 `src/strategies_5.py`（II/IV/V，`STRATEGIES_META` 常數）。
